class Solution {
    ...
    // https://leetcode.com/submissions/detail/741763881/
            ...
            if (houses[i] > 0) {
                ...
                for (int remaining = hr; remaining >= lr; remaining--) {
                    ...
                    if (houses[i - 1] > 0 && houses[i - 1] - 1 != j) {
                        dp[i][j][remaining - 1] = Math.min(dp[i][j][remaining - 1],
                                dp[i - 1][houses[i - 1] - 1][remaining]);
                    } else {
                ...
                for (int j = 0; j < n; j++) {
                    for (int remaining = hr; remaining >= lr; remaining--) {
                        ...
                        if (houses[i - 1] > 0 && houses[i - 1] - 1 != j) {
                            dp[i][j][remaining - 1] = Math.min(dp[i][j][remaining - 1],
                                    dp[i - 1][houses[i - 1] - 1][remaining] + cost[i][j]);
                        } else {
                            dp[i][j][remaining - 1] = Math.min(dp[i][j][remaining - 1],
                                    min(dp[i - 1], j, remaining) + cost[i][j]);
                        }
                    }
